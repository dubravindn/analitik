...
def _resolve_thread_id(explicit: object) -> str:
    """message_thread_id для доставки: явный аргумент или тема активного апдейта."""
    if explicit not in (None, "", 0, "0"):
        return str(explicit).strip()
    try:
        from .telegram import current_reply_thread
        return current_reply_thread() or ""
    except Exception:
        return ""


def enqueue_analysis(
    conn, payload: dict[str, Any], chat_id: str, *,
    force_mode: str | None = None, thread_id: object = None,
) -> str | None:
    run_mode = force_mode or mode()
    if run_mode == "off":
        return None
    thread = _resolve_thread_id(thread_id)
...
def _consume_result(conn_factory: Callable, bot_token: str, path: Path) -> None:
    ...
    try:
        ...
        thread_id = str(result.get("thread_id") or "").strip() or None

        def _thread_for(target: str) -> str | None:
            # message_thread_id имеет смысл только для форум-группы (id < 0).
            # Для личного чата руководителя Telegram вернёт ошибку.
            return thread_id if target.startswith("-") else None
        if run_mode == "shadow":
            ...
            for target in str(result.get("chat_id") or "").split(","):
                if target.strip():
                    sent = tg.send_message(
                        ...
                        message_thread_id=_thread_for(target.strip()),
                    )
